...
def filecreator():
    global fileNumber
    fileNumber = "0"
    while (os.path.exists(datetime.datetime.now().strftime("%y""%m""%d") + "-" + fileNumber + ".txt") == True):
        fileNumber = int(fileNumber) + 1
        fileNumber = str(fileNumber)
    file = open(datetime.datetime.now().strftime("%y""%m""%d") + "-" + fileNumber + ".txt", "a")
    file = open(datetime.datetime.now().strftime("%y""%m""%d")+"-"+ fileNumber +".txt", "a")
